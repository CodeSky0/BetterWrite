...
import { MaterialSheet } from '../../../components/student/MaterialSheet';
...
export default function TaskWritePage() {
  ...
  const handleQuoteMaterial = useCallback((quote: string) => {
    setContent((prev) => {
      const trimmed = quote.trim();
      if (!prev || prev.trim().length === 0) return trimmed;
      return `${prev.trim()}\n\n${trimmed}`;
    });
  ...
  return (
    ...
    >
      ...
      <MaterialSheet
        ...
        colors={colors}
        stage={(task?.stage as never) ?? undefined}
        topicType={task?.topicType}
        onQuote={handleQuoteMaterial}
      />
    </KeyboardAvoidingView>
  );
}
